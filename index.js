...
function customAssign(...objects) {
  if (objects.length < 2) { 
    return "Помилка: Має бути передано принаймні два об'єкти." // Перевірка, чи передано достатньо об'єктів якщо ні повертаємо "Помилка: Має бути передано принаймні два об'єкти."
  }
  const mergedObject = Object.assign({}, ...objects); // Використання методу `assign` для об'єднання об'єктів
  for (const key in mergedObject) { // Перетворення числових значень на рядки, ітеруємо об'єкт за допомогою for in
    if (typeof mergedObject[key] === "number") { // якщо значення типу число конвертуємо його в рядок
      mergedObject[key] = String(mergedObject[key]); 
    }
  }
  const transformedObject = {};
  for (const key in mergedObject) {
    transformedObject[key.replace(/ /g, "_")] = mergedObject[key];
  }
    return transformedObject;// Повернення нового об'єкта з об'єднаними та зміненими властивостями
}
...
function customEntries(obj) {
  if (typeof obj !== 'object' || obj === null) { 
    ...
    const entriesArray = Object.entries(obj);  // Використання методу `entries` для отримання масиву з усіма властивостями та значеннями
  return entriesArray;  // Повернення масиву властивостей та значень
}
...
function customObjectFromEntries(entries) {
  if (!Array.isArray(entries)) { 
    return "Помилка: Вхідний аргумент має бути масивом."; // Перевірка, чи вхідний аргумент є масивом,якщо ні повертаєм "Помилка: Вхідний аргумент має бути масивом."
  }
  const processedEntries = entries.map(([key, value]) => { // Використання методу `map` для обробки значень властивостей
    if (typeof key === 'number') { // Перевірка, чи ключ  є числом
    key = key.toString().replace(/ /g, '_'); // Перетворення числового значення на рядок
    }
    return [key, value]; 
    }); // Повернення обробленого запису [key, value]
  const objectFromEntries = Object.fromEntries(processedEntries); // Використання методу `fromEntries` для створення об'єкта з масиву записів
  return objectFromEntries; // Повернення створеного об'єкта
}
...
function checkProperty(obj, prop) {
  if (typeof obj !== 'object' || obj === null || Array.isArray(obj)) { 
    return false; // Перевіряємо, чи вхідний параметр obj є об'єктом, якщо ні, повертаємо false
  }
  if (typeof prop !== 'string') { // Перевіряємо, чи вхідний параметр prop є рядком, якщо ні, повертаємо false
    return false;
  }
    // Використовуємо метод hasOwnProperty() для перевірки наявності властивості в об'єкті
  return obj.hasOwnProperty(prop); // Повертаємо результат перевірки
}
...
function extendAndFreezeObject(obj1, obj2) {
  if (typeof obj1 !== 'object' || obj1 === null || Array.isArray(obj1)  || 
    ...
    return null; // Перевіряємо, чи вхідні параметри є об'єктами, якщо ні, повертаємо null
      }
  ...
  if (Object.isFrozen(obj1)) { // Перевіряємо, чи об'єкт "заморожено"
    return obj1; // Повертаємо "заморожений" об'єкт, якщо все пройшло успішно
  } else {
    return "Помилка: Об`єкт не може бути заморожено."; // Повертаємо повідомлення про помилку, якщо об'єкт не було "заморожено"
  }
}
...
function keysAndValues(obj) {
  if (typeof obj !== 'object' || obj === null || Array.isArray(obj)) { 
    return null; // Перевіряємо, чи вхідний параметр є об'єктом, якщо ні, повертаємо null
  }
...
function filterObjectsByKey(arr, key) {
  if (!Array.isArray(arr)) {
    return []; // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо пустий масив
  }
  const filteredArr = arr.filter((obj) => obj.hasOwnProperty(key)); // Використовуємо метод filter() для вибірки об'єктів, що містять вказаний ключ
  return filteredArr; // Повертаємо новий масив з об'єктами, що містять вказаний ключ
}
...
function checkAndPreventExtensions(obj) {
  if (typeof obj !== 'object' || obj === null) { 
    return false; // Перевіряємо, чи вхідний параметр є об'єктом, якщо ні, повертаємо false
  }
  const isExtensible = Object.isExtensible(obj); // Перевіряємо, чи можна додати нові властивості до об'єкта
  if (isExtensible) { 
    Object.preventExtensions(obj); // Якщо можна, запобігаємо подальшому додаванню властивостей
  }
...
function checkOwnership(obj1, obj2, key) {
  if (obj1.hasOwnProperty(key) && obj2.hasOwnProperty(key)) { 
    ...
    return obj1[key] === obj2[key]; // Якщо обидва об'єкти мають вказаний ключ повертаємо результат порівняння їх значень.
  } else { // Якщо значення однакові, повернемо true.
...
function getObjectValuesSum(obj) {
  if (typeof obj !== 'object' || obj === null) { 
    return 0;  // Перевірка, чи є аргумент об'єктом,якщо ні повертаємо 0
  }
  const values = Object.values(obj); // Отримуємо всі значення об'єкта

  const sum = values.reduce((acc, curr) => acc + curr, 0); // Обчислюємо суму значень
...
function convertArrayToObj(arr) {
  if (!Array.isArray(arr)) { 
    return {}; // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо пустий об'єкт
  }
  const resultObj = {}; // Створюємо пустий об'єкт який записуємо в змінну
  for (const subArray of arr) {; // Проходимося по кожному підмасиву в масиві за допопмогою циклу for, лічильник від нуля до довжини масиву
  const [key, value] = subArray; // Розпаковуємо підмасив за допомогою деструктурізації на окремі змінні для ключа та значення
  if (resultObj.hasOwnProperty(key)) { 
    console.log(`У масиві є дубльований ключ: ${key}`); // Перевіряємо, чи існує вже ключ в об'єкті,якщо так виводимо в консоль повідомлення `У масиві є дубльований ключ: ${key}`
  }
  resultObj[key] = value; 
} // Додаємо ключ та значення до об'єкта
  const finalObj = Object.fromEntries(Object.entries(resultObj)); 
  return finalObj;  // Застосовуємо метод Object.fromEntries() для створення об'єкта
}
